 ...
 */
import { registerPollingSource, } from "../polling/polling-state.js";
// ─── Constants ───────────────────────────────────────────────────────────────
...
export function classifyLevel(ratio, thresholds) {
    if (ratio < 0)
...
export function takeSnapshot(thresholds = config.thresholds) {
    const perf = performance;
    if (perf.memory) {
        const { usedJSHeapSize, totalJSHeapSize, jsHeapSizeLimit } = perf.memory;
        const usageRatio = jsHeapSizeLimit > 0 ? usedJSHeapSize / jsHeapSizeLimit : -1;
        return {
            usedJSHeapSize,
            totalJSHeapSize,
            jsHeapSizeLimit,
 ...
 */
export function startMemoryMonitor(overrides = {}) {
