const DEEP_LINK_VIEWS = new Set(["prd", "hench-runs"]);
function resolveLegacyViewAlias(base, sub) {
    const normalizedBase = base.trim().toLowerCase();
    const normalizedSub = (sub ?? "").trim().toLowerCase();
    if ((normalizedBase === "rex-dashboard" || normalizedBase === "rex")
...
function normalizeHashRoute(hash) {
    ...
    if (decoded.startsWith("sourcevision/"))
        decoded = decoded.slice("sourcevision/".length);
    if (decoded.startsWith("sourcevision:"))
        decoded = decoded.slice("sourcevision:".length);
...
function normalizeHashView(base) {
    const normalized = base.trim().toLowerCase();
    if (normalized === "prmarkdown" || normalized === "pr_markdown")
        return "pr-markdown";
    return normalized;
}
export function parsePathnameRoute(pathname, validViews) {
    ...
    if (legacyAlias && validViews.has(legacyAlias))
        return { view: legacyAlias, subId: null };
    if (validViews.has(raw))
        return { view: raw, subId: null };
    if (slashIdx > 0) {
        const view = base;
        if (validViews.has(view) && sub && DEEP_LINK_VIEWS.has(view))
            return { view, subId: sub };
    }
    return null;
}
export function parseLegacyHashRoute(hash, validViews) {
    if (!hash)
        return null;
...
export function resolveLocationRoute(pathname, hash, validViews) {
