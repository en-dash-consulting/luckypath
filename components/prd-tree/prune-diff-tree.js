...
function hasAffectedDescendant(item, prunableIds) {
...
function collectAffectedPaths(items, prunableIds) {
    const ids = new Set();
    function walk(nodes) {
...
function EpicImpactBadge({ impact }) {
    ...
    const pctLabel = pctChange > 0
        ...
        : pctChange < 0
            ? `${pctChange}%`
            : "0%";
    return h("span", { class: "prune-diff-epic-impact" }, h("span", { class: "prune-diff-impact-before" }, `${impact.before.completed}/${impact.before.total}`), h("span", { class: "prune-diff-impact-arrow" }, "\u2192"), h("span", { class: "prune-diff-impact-after" }, `${impact.after.completed}/${impact.after.total}`), h("span", {
        class: `prune-diff-impact-pct${pctChange >= 0 ? " prune-diff-impact-positive" : " prune-diff-impact-negative"}`,
    }, pctLabel), h("span", { class: "prune-diff-impact-removed" }, `-${impact.removedCount} item${impact.removedCount !== 1 ? "s" : ""}`));
}
/** Single tree node in the diff view. */
function DiffNodeRow({ item, depth, isExpanded, hasChildren, isPrunable, isAffectedParent, epicImpact, onToggle, }) {
    ...
    stats && stats.total > 0
        ? h("span", { class: "prune-diff-progress" }, `${stats.completed}/${stats.total}`, h("span", { class: "prune-diff-pct" }, ` ${Math.round(ratio * 100)}%`))
        : null, 
    // Epic impact badge
    epicImpact
...
function DiffTreeNodes({ items, depth, expanded, prunableIds, epicImpactMap, onToggle, }) {
    return h(Fragment, null, items.map((item) => {
        ...
        return h("div", { key: item.id, class: "prune-diff-node" }, h(DiffNodeRow, {
            item,
            depth,
            isExpanded: isOpen,
            hasChildren,
            ...
            ? h("div", { class: "prune-diff-children", role: "group" }, h(DiffTreeNodes, {
                items: children,
                depth: depth + 1,
                expanded,
                prunableIds,
...
// ── Main Component ───────────────────────────────────────────────────
export function PruneDiffTree({ prunableIds, epicImpact, onLoad }) {
    const [doc, setDoc] = useState(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    const [expanded, setExpanded] = useState(new Set());
    const [filterMode, setFilterMode] = useState("affected");
    ...
    const epicImpactMap = useMemo(() => {
        const map = new Map();
        for (const entry of epicImpact)
            map.set(entry.id, entry);
        return map;
    }, [epicImpact]);
    // Filter tree to only show affected branches
    const filteredItems = useMemo(() => {
        if (!doc || filterMode === "all")
            return doc?.items ?? [];
        function filterAffected(items) {
            const result = [];
    ...
    const allIds = useMemo(() => doc ? collectAllIds(doc.items) : new Set(), [doc]);
    ...
    // Loading state
    if (loading) {
        return h("div", { class: "prune-diff-loading" }, h("div", { class: "rex-analyze-spinner" }), h("span", null, "Loading tree diff..."));
    }
    // Error state
    if (error) {
    ...
    h("div", { class: "prune-diff-header" }, h("h4", null, "Tree Diff"), h("div", { class: "prune-diff-controls" }, 
    // Filter toggle
    h("button", {
    ...
    }, "Full Tree"), h("span", { class: "prune-diff-separator" }), h("button", {
    ...
    // Legend
    h("div", { class: "prune-diff-legend" }, h("span", { class: "prune-diff-legend-item prune-diff-legend-remove" }, h("span", { class: "prune-diff-indicator prune-diff-indicator-remove" }, "\u2212"), "Will be removed"), h("span", { class: "prune-diff-legend-item prune-diff-legend-affected" }, h("span", { class: "prune-diff-indicator prune-diff-indicator-modified" }, "\u2022"), "Children affected")), 
    // Overall completion impact
    epicImpact.length > 0
        ? h("div", { class: "prune-diff-overall-impact" }, h("span", { class: "prune-diff-overall-label" }, "Overall Completion:"), h("span", { class: "prune-diff-impact-before" }, `${pctBefore}%`), h("span", { class: "prune-diff-impact-arrow" }, "\u2192"), h("span", {
            class: `prune-diff-impact-after${pctAfter !== pctBefore ? " prune-diff-impact-changed" : ""}`,
        }, `${pctAfter}%`), pctAfter !== pctBefore
            ? h("span", { class: "prune-diff-impact-delta" }, `(${pctAfter > pctBefore ? "+" : ""}${pctAfter - pctBefore}pp)`)
            : null)
        : null, 
    // Tree
    h("div", { class: "prune-diff-tree", role: "tree", "aria-label": "Prune diff tree" }, h(DiffTreeNodes, {
        items: filteredItems,
        depth: 0,
        expanded,
        prunableIds,
...
//# sourceMappingURL=prune-diff-tree.js.map