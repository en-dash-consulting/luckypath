...
function isValidParent(childLevel, parentLevel) {
...
// ── Component ────────────────────────────────────────────────────────
export function AddItemForm({ allItems, onSubmit, onCancel, defaultParentId }) {
    const [title, setTitle] = useState("");
    const [description, setDescription] = useState("");
    const [level, setLevel] = useState(defaultParentId ? "" : "epic");
    ...
    const levelLabel = effectiveLevel
        ...
        : "item";
    return h("form", { class: "rex-add-form", onSubmit: handleSubmit }, 
    // Header
    h("div", { class: "rex-add-form-header" }, h("h3", { class: "rex-add-form-title" }, "Add New Item"), h("button", {
        type: "button",
    ...
    error
        ...
        : null, 
    // Level selector
    h("div", { class: "rex-add-form-field" }, h("label", { class: "rex-add-form-label", id: "add-form-type-label" }, "Type"), h("div", { class: "rex-add-form-level-group", role: "group", "aria-labelledby": "add-form-type-label" }, LEVEL_OPTIONS.map((opt) => h("button", {
        key: opt.value,
        type: "button",
    ...
    effectiveLevel !== "epic"
        ? h("div", { class: "rex-add-form-field" }, h("label", { class: "rex-add-form-label", for: "add-form-parent" }, "Parent"), h("select", {
            id: "add-form-parent",
            ...
            : h("option", { value: "", disabled: true }, "Select a parent..."), validParents.map((f) => h("option", { key: f.item.id, value: f.item.id }, `${"  ".repeat(f.depth)}${f.item.title} (${f.item.level})`))))
        : null, 
    // Title
    h("div", { class: `rex-add-form-field${titleError ? " rex-add-form-field-error" : ""}` }, h("label", { class: "rex-add-form-label", for: "add-form-title" }, "Title"), h("input", {
        id: "add-form-title",
        ...
        : null), 
    // Description
    h("div", { class: "rex-add-form-field" }, h("label", { class: "rex-add-form-label", for: "add-form-description" }, "Description ", h("span", { class: "rex-add-form-optional" }, "(optional)")), h("textarea", {
    ...
    // Priority
    h("div", { class: "rex-add-form-field" }, h("label", { class: "rex-add-form-label", for: "add-form-priority" }, "Priority"), h("select", {
    ...
    // Actions
    h("div", { class: "rex-add-form-actions" }, h("button", {
        type: "button",
...
//# sourceMappingURL=add-item-form.js.map