...
export function FacetFilter({ availableTags, activeTags, activeStatuses, onTagsChange, onStatusesChange, onClearAll, }) {
    ...
    const suggestions = useMemo(() => {
        ...
        const filtered = availableTags.filter((tag) => !activeTags.has(tag) && (!q || tag.toLowerCase().includes(q)));
        return filtered.slice(0, MAX_SUGGESTIONS);
    }, [availableTags, activeTags, tagQuery]);
    const totalMatches = useMemo(() => {
        const q = tagQuery.trim().toLowerCase();
        return availableTags.filter((tag) => !activeTags.has(tag) && (!q || tag.toLowerCase().includes(q))).length;
    }, [availableTags, activeTags, tagQuery]);
    ...
    return h("div", { class: "prd-facet-filter", role: "group", "aria-label": "Search facets" }, 
    // ── Status facets ─────────────────────────────────────────────────
    h("div", { class: "prd-facet-row" }, h("span", { class: "prd-facet-label" }, "Status:"), h("div", { class: "prd-facet-chips", role: "toolbar", "aria-label": "Status facets" }, STATUS_FACETS.map((sf) => {
        ...
        return h("button", {
            ...
            type: "button",
        }, h("span", { class: "prd-facet-chip-icon" }, sf.icon), h("span", { class: "prd-facet-chip-label" }, sf.label));
    }))), 
    // ── Tag facets (typeahead) ────────────────────────────────────────
    h("div", { class: "prd-facet-row prd-facet-row--tags" }, h("span", { class: "prd-facet-label" }, "Tags:"), availableTags.length > 0
        ? h("div", { class: "prd-tag-typeahead", ref: containerRef }, 
        // Selected tag chips
        activeTags.size > 0
            ? h("div", { class: "prd-facet-chips prd-tag-selected", role: "toolbar", "aria-label": "Active tag filters" }, [...activeTags].sort().map((tag) => h("button", {
                key: tag,
                ...
                "aria-pressed": "true",
                type: "button",
            }, h("span", { class: "prd-facet-chip-icon" }, "#"), h("span", { class: "prd-facet-chip-label" }, tag), h("span", { class: "prd-tag-remove" }, "\u00d7"))))
            : null, 
        // Search input
        h("div", { class: "prd-tag-input-wrap" }, h("input", {
            ref: inputRef,
            type: "text",
            class: "prd-tag-input",
        ...
        dropdownOpen && tagQuery.trim().length > 0 && suggestions.length > 0
            ? h("ul", { class: "prd-tag-dropdown", role: "listbox" }, suggestions.map((tag, idx) => h("li", {
                key: tag,
            ...
            }, h("span", { class: "prd-tag-option-hash" }, "#"), tag)), totalMatches > MAX_SUGGESTIONS
                ? h("li", { class: "prd-tag-option prd-tag-more" }, `+${totalMatches - MAX_SUGGESTIONS} more\u2026 refine your search`)
                : null)
            : dropdownOpen && tagQuery.trim().length > 0 && suggestions.length === 0
                ? h("ul", { class: "prd-tag-dropdown", role: "listbox" }, h("li", { class: "prd-tag-option prd-tag-no-match" }, "No matching tags"))
                : null))
