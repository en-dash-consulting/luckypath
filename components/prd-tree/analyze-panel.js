...
// ── Component ────────────────────────────────────────────────────────
export function AnalyzePanel({ onPrdChanged }) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (data.proposals && data.proposals.length > 0) {
                    ...
                    setState("done");
                    // Pre-select all
                    setSelectedIndices(new Set(data.proposals.map((_, i) => i)));
                }
            }
        }
        catch {
            ...
            if (data.proposals && data.proposals.length > 0) {
                ...
                if (pendingRes.ok) {
                    ...
                    if (pendingData.proposals && pendingData.proposals.length > 0) {
                        ...
                        setSelectedIndices(new Set(pendingData.proposals.map((_, i) => i)));
                        setState("done");
                        return;
                    }
                }
    ...
    const toggleProposal = useCallback((index) => {
    ...
    if (editing && proposals.length > 0) {
        return h("div", { class: "rex-analyze-panel" }, h(ProposalEditor, {
            proposals: proposals,
            onAccepted: handleEditorAccepted,
            onCancel: () => setEditing(false),
        }));
    }
    return h("div", { class: "rex-analyze-panel" }, 
    // Header
    h("div", { class: "rex-analyze-header" }, h("h3", { class: "rex-analyze-title" }, "Analyze Project")), 
    // Controls
    h("div", { class: "rex-analyze-controls" }, h("label", { class: "rex-analyze-checkbox-label" }, h("input", {
        type: "checkbox",
        checked: noLlm,
        onChange: () => setNoLlm(!noLlm),
    }), " Skip LLM refinement (faster, less refined)"), h("button", {
    ...
    // Progress indicator
    state === "running"
        ? h("div", { class: "rex-analyze-progress", role: "status", "aria-live": "polite" }, h("div", { class: "rex-analyze-spinner", "aria-hidden": "true" }), h("span", null, "Scanning project and building proposals..."))
        : null, 
    // Error display
    error
    ...
    state === "done" && proposals.length === 0
        ? h("div", { class: "rex-analyze-empty" }, h("p", null, "No new proposals found."), h("p", { class: "rex-analyze-hint" }, "The project may already be fully tracked in the PRD."))
        : null, state === "done" && proposals.length > 0
        ? h(Fragment, null, 
        // Selection controls
        h("div", { class: "rex-analyze-selection" }, h("span", { class: "rex-analyze-count" }, `${selectedIndices.size} of ${proposals.length} selected`), h("button", {
        ...
        // Proposal list
        h("div", { class: "rex-analyze-proposals" }, proposals.map((proposal, index) => h(ProposalCard, {
            key: index,
            proposal,
            index,
        ...
        h("div", { class: "rex-analyze-accept-bar" }, h("button", {
...
function ProposalCard({ proposal, index, selected, onToggle }) {
    ...
    return h("div", { class: `rex-proposal-card${selected ? " selected" : ""}` }, 
    // Card header
    h("div", {
    ...
    }), h("div", { class: "rex-proposal-info" }, h("span", { class: "prd-level-badge prd-level-epic" }, "Epic"), h("span", { class: "rex-proposal-title" }, proposal.epic.title)), h("span", { class: "rex-proposal-count" }, `${proposal.features.length} features, ${taskCount} tasks`), h("button", {
    ...
    // Expanded details
    expanded
        ? h("div", { class: "rex-proposal-details" }, proposal.epic.description
            ? h("p", { class: "rex-proposal-desc" }, proposal.epic.description)
            : null, proposal.features.map((feature, fi) => h("div", { key: fi, class: "rex-proposal-feature" }, h("div", { class: "rex-proposal-feature-header" }, h("span", { class: "prd-level-badge prd-level-feature" }, "Feature"), h("span", null, feature.title)), feature.tasks.length > 0
            ? h("div", { class: "rex-proposal-tasks" }, feature.tasks.map((task, ti) => h("div", { key: ti, class: "rex-proposal-task" }, h("span", { class: "prd-level-badge prd-level-task" }, "Task"), h("span", { class: "rex-proposal-task-title" }, task.title), task.priority
                ? h("span", { class: `prd-priority-badge prd-priority-${task.priority}` }, task.priority)
                : null)))
            : null)))
        : null);
}
//# sourceMappingURL=analyze-panel.js.map