...
// ── Component ────────────────────────────────────────────────────
export function ReorganizePanel({ open, onClose, onApplied }) {
    const [proposals, setProposals] = useState([]);
    const [llmProposals, setLlmProposals] = useState([]);
    const [loading, setLoading] = useState(false);
    const [applying, setApplying] = useState(false);
    const [selected, setSelected] = useState(new Set());
    const [selectedLlm, setSelectedLlm] = useState(new Set());
    ...
    const toggleLlmSelection = useCallback((id) => {
        setSelectedLlm((prev) => {
            const next = new Set(prev);
            if (next.has(id)) {
                next.delete(id);
    ...
    const applySelected = useCallback(async () => {
        if (selected.size === 0 && selectedLlm.size === 0)
            return;
        setApplying(true);
        setError(null);
        try {
    ...
    return h("div", { class: "reorg-overlay" }, h("div", { class: "reorg-panel" }, 
    // Header
    h("div", { class: "reorg-header" }, h("h3", null, "Reorganize PRD"), h("button", { class: "reorg-close", onClick: onClose, "aria-label": "Close" }, "×")), 
    // Content
    h("div", { class: "reorg-body" }, error ? h("div", { class: "reorg-error" }, error) : null, result ? h("div", { class: "reorg-success" }, result) : null, loading
        ...
        : !hasProposals && !hasLlmProposals
            ...
            hasProposals
                ? h("div", null, h("div", { class: "reorg-section-header" }, "Structural Proposals"), proposals.map((p) => h("label", {
                    key: `s-${p.id}`,
                ...
                }), h("div", { class: "reorg-card-content" }, h("div", { class: "reorg-card-top" }, h("span", { class: "reorg-type-icon" }, TYPE_ICONS[p.type] ?? "?"), h("span", { class: "reorg-type-label" }, p.type), h("span", { class: `reorg-risk ${RISK_CLASSES[p.risk] ?? ""}` }, p.risk), h("span", { class: "reorg-confidence" }, `${Math.round(p.confidence * 100)}%`)), h("div", { class: "reorg-description" }, p.description), p.items.length > 0
                    ? h("div", { class: "reorg-affected" }, `Affects: ${p.items.join(", ")}`)
                    : null))))
                : null, 
            // LLM proposals section
            hasLlmProposals
                ? h("div", null, h("div", { class: "reorg-section-header" }, "LLM Proposals"), llmProposals.map((p) => h("label", {
                    key: `l-${p.id}`,
                    ...
                    class: "reorg-checkbox",
                }), h("div", { class: "reorg-card-content" }, h("div", { class: "reorg-card-top" }, h("span", { class: "reorg-type-icon" }, LLM_ACTION_ICONS[p.action] ?? "?"), h("span", { class: "reorg-type-label" }, p.action), h("span", { class: "reorg-source-badge" }, "LLM")), h("div", { class: "reorg-description" }, p.reason)))))
                : null)), 
    // Footer
    (hasProposals || hasLlmProposals)
        ? h("div", { class: "reorg-footer" }, lowRiskCount > 0
            ? h("button", {
...
//# sourceMappingURL=reorganize-panel.js.map