...
// ── Component ────────────────────────────────────────────────────────
export function SmartAddInput({ onPrdChanged, compact }) {
    ...
    // ── Debounced preview ────────────────────────────────────────────
    const triggerPreview = useCallback(async (text, reqId, parentId) => {
    ...
    if (editing && proposals.length > 0) {
        return h("div", { class: "smart-add-panel" }, h(ProposalEditor, {
            proposals,
    ...
    const taskCount = proposals.reduce((sum, p) => sum + p.features.reduce((fs, f) => fs + f.tasks.length, 0), 0);
    ...
    // Header
    h("div", { class: "smart-add-header" }, h("h3", { class: "smart-add-title" }, "Smart Add"), h("p", { class: "smart-add-subtitle" }, "Describe what you want to build, then click Generate.")), 
    // Context selection dropdown
    scopeOptions.length > 0
        ? h("div", { class: "smart-add-scope" }, h("label", { class: "smart-add-scope-label" }, "Scope"), h("select", {
        ...
        }, h("option", { value: "" }, "Entire project (new epics)"), scopeOptions.map((opt) => h("option", { key: opt.id, value: opt.id }, `${"  ".repeat(opt.depth)}${getLevelLabel(opt.level)}: ${opt.title}`))), selectedScope
            ? h("span", { class: "smart-add-scope-active" }, `Adding under ${scopeLabel}`)
            : null)
        : null, 
    // Example prompts (shown when idle and no input)
    state === "idle" && trimmedLength === 0
        ? h("div", { class: "smart-add-examples" }, h("span", { class: "smart-add-examples-label" }, "Try something like:"), h("div", { class: "smart-add-examples-list" }, EXAMPLE_PROMPTS.map((example, i) => h("button", {
            key: i,
            type: "button",
        ...
        : null, 
    // Input area
    h("div", { class: "smart-add-input-area" }, h("textarea", {
    ...
    trimmedLength > 0 && trimmedLength < MIN_INPUT_LENGTH
        ? h("span", { class: "smart-add-hint" }, `${MIN_INPUT_LENGTH - trimmedLength} more to generate proposals`)
        : null, 
    // Loading badge
    state === "loading"
        ? h("span", { class: "smart-add-loading-badge" }, h("span", { class: "smart-add-spinner", "aria-hidden": "true" }), " Generating...")
        : null), 
    ...
    state === "loading"
        ? h("div", { class: "smart-add-loading", role: "status", "aria-live": "polite" }, h("div", { class: "smart-add-loading-bar" }, h("div", { class: "smart-add-loading-bar-fill" })), h("span", { class: "smart-add-loading-text" }, "Analyzing description and generating proposals..."))
        : null, 
    // Error display
    error
        ? h("div", { class: "smart-add-error", role: "alert", "aria-live": "assertive" }, h("span", { class: "smart-add-error-icon" }, "\u26A0"), h("span", null, error))
        : null, 
    // Results: proposal preview
    state === "done" && proposals.length === 0
        ? h("div", { class: "smart-add-empty" }, h("p", null, "No proposals generated."), h("p", { class: "smart-add-empty-hint" }, "Try a more detailed description or different phrasing."))
        : null, state === "done" && proposals.length > 0
        ? h(Fragment, null, 
        // Confidence bar
        h(ConfidenceBar, { confidence }), 
        // Quality issues (if any)
        qualityIssues.length > 0
            ? h("div", { class: "smart-add-quality-issues" }, h("span", { class: "smart-add-quality-icon" }, "\u26A0"), h("span", null, `${qualityIssues.length} quality warning${qualityIssues.length !== 1 ? "s" : ""}`))
            : null, 
        // Summary stats
        h("div", { class: "smart-add-stats" }, h("span", { class: "smart-add-stat" }, h("span", { class: "smart-add-stat-count" }, String(proposals.length)), ` epic${proposals.length !== 1 ? "s" : ""}`), h("span", { class: "smart-add-stat-sep" }, "\u2022"), h("span", { class: "smart-add-stat" }, h("span", { class: "smart-add-stat-count" }, String(featureCount)), ` feature${featureCount !== 1 ? "s" : ""}`), h("span", { class: "smart-add-stat-sep" }, "\u2022"), h("span", { class: "smart-add-stat" }, h("span", { class: "smart-add-stat-count" }, String(taskCount)), ` task${taskCount !== 1 ? "s" : ""}`)), 
        // Proposal hierarchy preview
        h("div", { class: "smart-add-preview" }, proposals.map((proposal, pi) => h(ProposalPreviewCard, { key: pi, proposal, index: pi }))), 
        // Action buttons
        h("div", { class: "smart-add-actions" }, h("button", {
            class: "smart-add-btn smart-add-btn-review",
            onClick: () => setEditing(true),
            disabled: accepting,
            type: "button",
        }, "\u270E Review & Edit"), h("button", {
...
function ConfidenceBar({ confidence }) {
    const level = confidence >= 80 ? "high" :
        confidence >= 50 ? "medium" :
            "low";
    const label = level === "high" ? "High confidence" :
        level === "medium" ? "Moderate confidence" :
            "Low confidence";
    return h("div", { class: "smart-add-confidence" }, h("div", { class: "smart-add-confidence-header" }, h("span", { class: "smart-add-confidence-label" }, label), h("span", { class: `smart-add-confidence-value smart-add-confidence-${level}` }, `${confidence}%`)), h("div", { class: "smart-add-confidence-track" }, h("div", {
...
function ProposalPreviewCard({ proposal, index }) {
    const [expanded, setExpanded] = useState(true);
    const taskCount = proposal.features.reduce((sum, f) => sum + f.tasks.length, 0);
    return h("div", { class: "smart-add-preview-card" }, 
    // Epic header
    h("div", {
        ...
        "aria-hidden": "true",
    }, "\u25B6"), h("span", { class: "prd-level-badge prd-level-epic" }, "Epic"), h("span", { class: "smart-add-preview-title" }, proposal.epic.title), h("span", { class: "smart-add-preview-count" }, `${proposal.features.length}F / ${taskCount}T`)), 
    // Expanded content
    expanded
        ? h("div", { class: "smart-add-preview-body" }, proposal.epic.description
            ? h("p", { class: "smart-add-preview-desc" }, proposal.epic.description)
            : null, proposal.features.map((feature, fi) => h("div", { key: fi, class: "smart-add-preview-feature" }, h("div", { class: "smart-add-preview-feature-header" }, h("span", { class: "prd-level-badge prd-level-feature" }, "Feature"), h("span", { class: "smart-add-preview-feature-title" }, feature.title), feature.description
            ? h("span", { class: "smart-add-preview-feature-desc" }, ` \u2014 ${feature.description}`)
            : null), feature.tasks.length > 0
            ? h("div", { class: "smart-add-preview-tasks" }, feature.tasks.map((task, ti) => h("div", { key: ti, class: "smart-add-preview-task" }, h("span", { class: "prd-level-badge prd-level-task" }, "Task"), h("span", { class: "smart-add-preview-task-title" }, task.title), task.priority
                ? h("span", {
                    class: `prd-priority-badge prd-priority-${task.priority}`,
                }, task.priority)
                : null, task.acceptanceCriteria && task.acceptanceCriteria.length > 0
                ? h("span", { class: "smart-add-preview-ac-badge" }, `${task.acceptanceCriteria.length} AC`)
                : null)))
            : null)))
        : null);
}
//# sourceMappingURL=smart-add-input.js.map