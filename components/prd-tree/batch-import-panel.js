...
// ── Component ────────────────────────────────────────────────────────
export function BatchImportPanel({ onPrdChanged }) {
    ...
    if (editing && proposals.length > 0) {
        return h("div", { class: "batch-import-panel" }, h(ProposalEditor, {
            proposals,
    ...
    if (state === "done" && proposals.length > 0) {
        ...
        return h("div", { class: "batch-import-panel" }, 
        // Header
        h("div", { class: "batch-import-header" }, h("h3", { class: "batch-import-title" }, "Batch Import Results"), h("p", { class: "batch-import-subtitle" }, `Processed ${items.length} item${items.length !== 1 ? "s" : ""}. Review the proposals below.`)), 
        // Summary stats
        ...
        confidence > 0
            ? h("div", { class: "smart-add-confidence" }, h("div", { class: "smart-add-confidence-header" }, h("span", { class: "smart-add-confidence-label" }, confidence >= 80 ? "High confidence" :
                confidence >= 50 ? "Moderate confidence" :
                    "Low confidence"), h("span", {
                class: `smart-add-confidence-value smart-add-confidence-${confidence >= 80 ? "high" : confidence >= 50 ? "medium" : "low"}`,
            }, `${confidence}%`)), h("div", { class: "smart-add-confidence-track" }, h("div", {
        ...
        h("div", { class: "smart-add-actions" }, h("button", {
            ...
            ? "Accepting..."
            : `Accept All (${proposals.length + featureCount + taskCount} items)`), h("button", {
            class: "smart-add-btn smart-add-btn-review",
            onClick: () => {
                setProposals([]);
                setConfidence(0);
    ...
    return h("div", { class: "batch-import-panel" }, 
    // Header
    h("div", { class: "batch-import-header" }, h("h3", { class: "batch-import-title" }, "Batch Import"), h("p", { class: "batch-import-subtitle" }, "Upload files or paste ideas. All items are processed together and deduplicated.")), 
    // Drop zone
    h("div", {
    ...
    }, h("div", { class: "batch-import-dropzone-content" }, h("span", { class: "batch-import-dropzone-icon" }, "\uD83D\uDCC1"), h("p", { class: "batch-import-dropzone-text" }, "Drop files here or ", h("button", {
        class: "batch-import-browse-btn",
        onClick: () => fileInputRef.current?.click(),
        type: "button",
    }, "browse")), h("p", { class: "batch-import-dropzone-hint" }, "Supports .txt, .md, .json files")), h("input", {
        ref: fileInputRef,
        type: "file",
        multiple: true,
    ...
    // Queued items
    items.length > 0
        ? h("div", { class: "batch-import-queue" }, h("div", { class: "batch-import-queue-header" }, h("span", { class: "batch-import-queue-title" }, `${items.length} item${items.length !== 1 ? "s" : ""} queued`), h("button", {
            class: "batch-import-clear-btn",
            onClick: () => setItems([]),
            type: "button",
        }, "Clear all")), items.map((item) => h(BatchItemRow, {
            key: item.id,
            item,
            onUpdate: updateItem,
            onRemove: removeItem,
    ...
    // Error display
    error
        ? h("div", { class: "smart-add-error", role: "alert" }, h("span", { class: "smart-add-error-icon" }, "\u26A0"), h("span", null, error))
        : null, 
    // Processing state — stage-based progress
    state === "processing"
        ? h("div", { class: "batch-import-progress", role: "status", "aria-live": "polite" }, h("div", { class: "batch-import-progress-bar" }, h("div", {
            ...
            "aria-valuemin": 0,
            "aria-valuemax": 100,
        })), h("div", { class: "batch-import-progress-info" }, h("span", { class: "batch-import-progress-text" }, STAGE_LABELS[processingStage]), h("span", { class: "batch-import-progress-detail" }, `${validCount} file${validCount !== 1 ? "s" : ""}`)), h("div", { class: "batch-import-progress-stages" }, ["uploading", "analyzing", "generating"].map((stage) => {
            ...
            }, isDone ? "\u2713 " : isCurrent ? "\u25CF " : "\u25CB ", stage === "uploading" ? "Upload" :
                stage === "analyzing" ? "Analyze" : "Generate");
        })))
        : null, 
    // Empty result
    state === "done" && proposals.length === 0
        ? h("div", { class: "smart-add-empty" }, h("p", null, "No proposals generated from the imported items."), h("p", { class: "smart-add-empty-hint" }, "Try adding more detailed content or different formats."))
        : null, 
    // Process button
    validCount > 0 && state !== "processing"
        ? h("div", { class: "batch-import-process" }, h("button", {
...
export function BatchItemRow({ item, onUpdate, onRemove }) {
    ...
    return h("div", { class: "batch-import-item" }, 
    // Item header
    h("div", { class: "batch-import-item-header" }, h("span", { class: `batch-import-format-badge batch-import-format-${item.format}` }, item.format.toUpperCase()), h("span", { class: "batch-import-item-source" }, item.source), !isTextEntry
        ? h("span", { class: "batch-import-item-meta" }, h("span", { class: "batch-import-item-size" }, formatSize(size)), h("span", { class: "batch-import-item-meta-sep" }, "\u00B7"), h("span", null, `${lines} line${lines !== 1 ? "s" : ""}`), h("span", { class: "batch-import-item-meta-sep" }, "\u00B7"), h("span", null, `${words} word${words !== 1 ? "s" : ""}`))
        : null, h("select", {
    ...
    }, h("option", { value: "text" }, "Text"), h("option", { value: "markdown" }, "Markdown"), h("option", { value: "json" }, "JSON")), 
    ...
    isTextEntry
        ...
        : // Preview for file content — collapsed or expanded
            h("div", {
                ...
                ? item.content
                : h(Fragment, null, item.content.slice(0, 300), item.content.length > 300 ? "\u2026" : "")));
}
//# sourceMappingURL=batch-import-panel.js.map