...
export function Breadcrumb({ view, navigateTo, scope }) {
    ...
    useEffect(() => {
        ...
        const parts = [];
        if (meta)
            parts.push(`${meta.label} — ${meta.productLabel}`);
        if (project)
    ...
    meta && meta.product !== "global"
        ? h("li", { class: "breadcrumb-item" }, h("button", {
            ...
            type: "button",
        }, meta.productLabel), Separator())
        : null, 
    // ── Segment 3: Current view (active, not a link) ──
    meta
        ? h("li", { class: "breadcrumb-item breadcrumb-current", "aria-current": "page" }, meta.label)
        : null));
}
