...
export function TreeView({ nodes, renderNode, defaultExpandDepth = 2, filterMatch, }) {
    const [expanded, setExpanded] = useState(() => {
        const set = new Set();
        function autoExpand(ns, depth) {
            for (const n of ns) {
    ...
    function renderNodes(ns, depth, parentIsLast = false) {
        return ns.map((node, idx) => {
            ...
            h("span", { class: "tree-node-content" }, renderNode(node, depth))), 
            // Children (indented)
            hasChildren && isOpen
                ? h("div", {
                    class: "tree-children",
...
function expandForFilter(nodes, matchSet) {
...
function nodeMatchesFilter(node, matchSet) {
