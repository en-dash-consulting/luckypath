 ...
 */
function highlightText(text, queryTerms, maxLength = 200) {
    ...
    for (const range of merged) {
        ...
        fragments.push(h("mark", { class: cls }, text.slice(range.start, range.end)));
        cursor = range.end;
    }
 ...
 */
function getContextSnippet(text, queryTerms) {
    if (!text)
        return null;
    const lower = text.toLowerCase();
    for (const term of queryTerms) {
...
// ── Component ──────────────────────────────────────────────────────────────
export function SearchOverlay({ visible, onClose, navigateTo }) {
    ...
    const filteredResults = useMemo(() => {
        ...
        return results.filter((r) => {
            if (filters.levels.size > 0 && !filters.levels.has(r.level))
                return false;
            if (filters.statuses.size > 0 && !filters.statuses.has(r.status))
                return false;
    ...
    // ── Search Input ──────────────────────────────────────────────
    h("div", { class: "search-overlay-header" }, h("div", { class: "search-overlay-input-row" }, h("span", { class: "search-overlay-icon", "aria-hidden": "true" }, "\u{1F50D}"), h("input", {
        ref: inputRef,
    ...
    // ── Filter Toggle ───────────────────────────────────────────
    h("div", { class: "search-overlay-filter-toggle" }, h("button", {
        ...
        : null), query && !loading
        ? h("span", { class: "search-overlay-stats", "aria-live": "polite" }, `${filteredResults.length} result${filteredResults.length !== 1 ? "s" : ""}`, elapsed != null ? ` (${elapsed}ms)` : "")
        : null)), 
    // ── Filters Panel ─────────────────────────────────────────────
    filtersExpanded
    ...
    h("div", {
        ...
        : query && filteredResults.length === 0
            ...
            : filteredResults.map((result, i) => renderResult(result, i, i === activeIndex, queryTerms, navigateToResult))), 
    // ── Footer ────────────────────────────────────────────────────
    h("div", { class: "search-overlay-footer" }, h("span", null, h("kbd", null, "\u2191\u2193"), " navigate  ", h("kbd", null, "\u21B5"), " select  ", h("kbd", null, "esc"), " close"))));
}
// ── Sub-renderers ──────────────────────────────────────────────────────────
function renderFilterGroup(label, group, options, active, onToggle) {
    return h("div", { class: "search-filter-group" }, h("span", { class: "search-filter-group-label" }, label), h("div", { class: "search-filter-group-options" }, ...options.map((opt) => h("label", {
...
function renderResult(result, index, isActive, queryTerms, onSelect) {
    ...
    h("div", { class: "search-result-header" }, h("span", { class: "search-result-level", title: result.level }, levelIcon), h("span", { class: "search-result-title" }, ...highlightText(result.title, queryTerms)), h("span", { class: statusClass }, statusLabel)), 
    // Breadcrumb parent chain
    result.parentChain.length > 0
        ? h("div", { class: "search-result-breadcrumb" }, result.parentChain.join(" \u203A "))
        : null, 
    // Context snippet with highlights
    contextSnippet
        ? h("div", { class: "search-result-context" }, ...highlightText(contextSnippet, queryTerms, 200))
        : null, 
    // Matched fields
    result.matchedFields.length > 0
        ? h("div", { class: "search-result-meta" }, "Matched: ", result.matchedFields.join(", "))
        : null);
}
