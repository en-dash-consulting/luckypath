...
const EVENT_DRIVEN_REASONS = new Set(["close", "error", "end", "shutdown"]);
...
export function WsHealthPanel() {
    ...
    if (!loaded)
        return null;
    const { activeConnections, peakConnections, totalConnectionsAccepted, totalConnectionsRemoved, cleanupsByReason, recentCleanups, avgConnectionDurationMs, totalBroadcasts, totalBroadcastWriteFailures, cleanupSuccessRate, health, uptimeMs, } = status;
    ...
    const cleanupEntries = Object.entries(cleanupsByReason)
    ...
    // Header row
    h("div", { class: "ws-health-header" }, h("div", { class: "ws-health-header-left" }, h("span", {
        class: `ws-health-icon ws-health-icon-${health}`,
        "aria-hidden": "true",
    }, config.icon), h("h3", { class: "ws-health-title" }, "WebSocket Health"), h("span", {
        class: `ws-health-badge ws-health-badge-${health}`,
    }, config.label)), h("div", { class: "ws-health-header-right" }, h("span", {
        class: "ws-health-connection-count",
        style: `color: ${config.color}`,
    }, h("span", { class: "ws-health-count-value" }, String(activeConnections)), h("span", { class: "ws-health-count-label" }, " active")))), 
    // Connection bar — active vs peak
    peakConnections > 0
        ? h("div", { class: "ws-health-bar-container" }, h("div", { class: "ws-health-bar-label" }, h("span", null, "Connections"), h("span", { class: "ws-health-bar-detail" }, `${activeConnections} active / ${peakConnections} peak`)), h("div", { class: "ws-health-bar-track" }, h("div", {
    ...
    totalConnectionsRemoved > 0
        ? h("div", { class: "ws-health-bar-container" }, h("div", { class: "ws-health-bar-label" }, h("span", null, "Cleanup Success"), h("span", { class: "ws-health-bar-detail" }, `${successPct}%`)), h("div", { class: "ws-health-bar-track" }, h("div", {
    ...
    cleanupEntries.length > 0
        ? h("div", { class: "ws-health-cleanup-breakdown" }, h("div", { class: "ws-health-cleanup-header" }, h("span", { class: "ws-health-cleanup-title" }, "Cleanup Reasons"), h("span", { class: "ws-health-cleanup-recent" }, `${recentCleanups} in last 60s`)), ...cleanupEntries.map(([reason, count]) => h("div", {
            key: reason,
            class: `ws-health-cleanup-row${EVENT_DRIVEN_REASONS.has(reason) ? "" : " ws-health-cleanup-row-warn"}`,
        }, h("span", { class: "ws-health-cleanup-reason" }, REASON_LABELS[reason]), h("span", { class: "ws-health-cleanup-count" }, String(count)))))
        : null, 
    // Stats row
    h("div", { class: "ws-health-stats" }, h("div", { class: "ws-health-stat" }, h("span", { class: "ws-health-stat-value" }, String(totalConnectionsAccepted)), h("span", { class: "ws-health-stat-label" }, "total accepted")), h("div", { class: "ws-health-stat" }, h("span", { class: "ws-health-stat-value" }, String(totalBroadcasts)), h("span", { class: "ws-health-stat-label" }, "broadcasts")), h("div", { class: "ws-health-stat" }, h("span", { class: "ws-health-stat-value" }, avgConnectionDurationMs > 0 ? formatDuration(avgConnectionDurationMs) : "—"), h("span", { class: "ws-health-stat-label" }, "avg duration")), h("div", { class: "ws-health-stat" }, h("span", { class: "ws-health-stat-value" }, formatUptime(uptimeMs)), h("span", { class: "ws-health-stat-label" }, "uptime"))), 
    // Broadcast failure indicator (only show if there are failures)
    totalBroadcastWriteFailures > 0
        ? h("div", { class: "ws-health-failure-alert" }, h("span", { class: "ws-health-failure-icon", "aria-hidden": "true" }, "△"), h("span", { class: "ws-health-failure-text" }, `${totalBroadcastWriteFailures} broadcast write failure${totalBroadcastWriteFailures === 1 ? "" : "s"} (${broadcastFailRate}% of broadcasts)`))
        : null);
}
//# sourceMappingURL=ws-health-panel.js.map