...
export function ConcurrencyPanel() {
    ...
    // Header row
    h("div", { class: "concurrency-header" }, h("div", { class: "concurrency-header-left" }, h("span", {
        class: `concurrency-icon concurrency-icon-${level}`,
        "aria-hidden": "true",
    }, config.icon), h("h3", { class: "concurrency-title" }, "Concurrency"), h("span", {
        class: `concurrency-level-badge concurrency-level-${level}`,
    }, config.label)), h("div", { class: "concurrency-header-right" }, h("span", {
        class: "concurrency-ratio",
        style: `color: ${config.color}`,
    }, h("span", { class: "concurrency-ratio-current" }, String(processCount)), h("span", { class: "concurrency-ratio-sep" }, "/"), h("span", { class: "concurrency-ratio-max" }, String(maxConcurrent))))), 
    // Utilization bar
    h("div", { class: "concurrency-bar-container" }, h("div", {
    ...
    ...Array.from({ length: maxConcurrent - 1 }, (_, i) => h("div", {
        key: i,
    ...
    // Stats row
    h("div", { class: "concurrency-stats" }, h("div", { class: "concurrency-stat" }, h("span", { class: "concurrency-stat-value" }, String(slotsAvailable)), h("span", { class: "concurrency-stat-label" }, `slot${slotsAvailable === 1 ? "" : "s"} available`)), h("div", { class: "concurrency-stat" }, h("span", { class: "concurrency-stat-value" }, String(totalRunning)), h("span", { class: "concurrency-stat-label" }, `task${totalRunning === 1 ? "" : "s"} running`)), pendingTasks > 0
        ? h("div", { class: "concurrency-stat" }, h("span", { class: "concurrency-stat-value concurrency-stat-pending" }, String(pendingTasks)), h("span", { class: "concurrency-stat-label" }, `pending task${pendingTasks === 1 ? "" : "s"}`))
        : null));
}
//# sourceMappingURL=concurrency-panel.js.map