...
export function MemoryPanel() {
    ...
    // Header row
    h("div", { class: "memory-header" }, h("div", { class: "memory-header-left" }, h("span", {
        class: `memory-icon memory-icon-${health}`,
        "aria-hidden": "true",
    }, config.icon), h("h3", { class: "memory-title" }, "Memory"), h("span", {
        class: `memory-health-badge memory-health-${health}`,
    }, config.label)), h("div", { class: "memory-header-right" }, h("span", {
        class: "memory-pct-display",
        style: `color: ${config.color}`,
    }, h("span", { class: "memory-pct-value" }, `${system.usedPercent}`), h("span", { class: "memory-pct-sign" }, "%")))), 
    // Memory pressure warning (only for warning/critical)
    health !== "healthy"
        ? h("div", { class: `memory-pressure-alert memory-pressure-${health}` }, h("span", { class: "memory-pressure-icon", "aria-hidden": "true" }, health === "critical" ? "▲" : "△"), h("span", { class: "memory-pressure-text" }, health === "critical"
            ? "System memory critically low — task performance may degrade"
            : "Memory usage elevated — monitor for potential issues"))
        : null, 
    // System memory bar
    h("div", { class: "memory-bar-container" }, h("div", { class: "memory-bar-label" }, h("span", null, "System"), h("span", { class: "memory-bar-detail" }, `${formatBytes(system.usedBytes)} / ${formatBytes(system.totalBytes)}`)), h("div", { class: "memory-bar-track" }, h("div", {
    ...
    h("div", { class: "memory-bar-container" }, h("div", { class: "memory-bar-label" }, h("span", null, "Server (n-dx)"), h("span", { class: "memory-bar-detail" }, formatBytes(server.rssBytes))), h("div", { class: "memory-bar-track" }, h("div", {
    ...
    processes.length > 0
        ? h("div", { class: "memory-processes" }, h("div", { class: "memory-processes-header" }, h("span", { class: "memory-processes-title" }, "Task Processes"), h("span", { class: "memory-processes-count" }, String(processes.length))), ...processes.map((proc) => h("div", { key: proc.taskId, class: "memory-process-row" }, h("div", { class: "memory-process-info" }, h("span", { class: "memory-process-name" }, proc.taskTitle), h("span", { class: "memory-process-pid" }, `PID ${proc.pid}`)), h("span", { class: "memory-process-rss" }, formatBytes(proc.rssBytes)))))
        : null, 
    // Stats row: load average and CPU info
    h("div", { class: "memory-stats" }, h("div", { class: "memory-stat" }, h("span", { class: "memory-stat-value" }, formatBytes(system.freeBytes)), h("span", { class: "memory-stat-label" }, "free")), h("div", { class: "memory-stat" }, h("span", { class: "memory-stat-value" }, loadAvg[0].toFixed(2)), h("span", { class: "memory-stat-label" }, "load (1m)")), h("div", { class: "memory-stat" }, h("span", { class: "memory-stat-value" }, String(cpuCount)), h("span", { class: "memory-stat-label" }, `CPU${cpuCount === 1 ? "" : "s"}`))));
}
//# sourceMappingURL=memory-panel.js.map