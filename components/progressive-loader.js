 ...
 */
export function countVisibleNodes(items, activeStatuses) {
    let count = 0;
 ...
 */
function sliceItems(items, activeStatuses, budget) {
    const result = [];
    for (const item of items) {
 ...
 */
function countVisibleChildren(children, activeStatuses) {
 ...
 */
export function sliceVisibleTree(items, activeStatuses, limit) {
 ...
 */
export function useProgressiveLoader(totalCount, chunkSize = DEFAULT_CHUNK_SIZE) {
    const isActive = totalCount > PROGRESSIVE_THRESHOLD;
    const [limit, setLimit] = useState(() => isActive ? chunkSize : totalCount);
 ...
 */
export function LoadMoreIndicator({ renderedCount, totalCount, chunkSize, isLoading, onLoadMore, onLoadAll, }) {
    const remaining = totalCount - renderedCount;
    const nextChunk = Math.min(remaining, chunkSize);
    return h("div", { class: "prd-load-more", role: "status", "aria-live": "polite" }, 
    // Progress info
    h("div", { class: "prd-load-more-info" }, `Showing ${renderedCount} of ${totalCount} nodes`), 
    // Progress bar
    h("div", {
    ...
    // Action buttons
    h("div", { class: "prd-load-more-actions" }, h("button", {
        class: "prd-load-more-btn prd-load-more-btn-primary",
        onClick: onLoadMore,
        disabled: isLoading,
        "aria-label": `Load ${nextChunk} more nodes`,
    }, isLoading ? "Loading\u2026" : `Load ${nextChunk} more`), h("button", {
