...
import { useToast, usePRDData, usePRDWebSocket, usePRDActions, usePRDDeepLink, usePersistentFilter, useFeatureToggle, useFacetState, } from "../hooks/index.js";
...
export function PRDView({ prdData, onSelectItem, onDetailContent, initialTaskId, navigateTo }) {
    // ── Toast notifications ────────────────────────────────────────
    const { toast, toastType, showToast } = useToast();
    // ── Data fetching & polling ────────────────────────────────────
    const { data, setData, loading, error, setError, taskUsageById, weeklyBudget, fetchPRDData, fetchTaskUsage, } = usePRDData(prdData);
    // ── Feature toggles ────────────────────────────────────────────
    ...
    const { activeTags, activeSearchStatuses, searchFacets, setActiveTags, setActiveSearchStatuses, clearFacets, hasFacets, } = useFacetState();
    // Collect all unique tags from the PRD for facet chip rendering
    const availableTags = useMemo(() => data ? collectAllTags(data.items) : [], [data]);
    ...
    const searchResult = useMemo(() => data ? searchTree(data.items, searchQuery, searchFacets) : null, [data, searchQuery, searchFacets]);
    ...
    const { deepLinkError, setDeepLinkError, highlightedTaskId, deepLinkExpandIds } = usePRDDeepLink({
    ...
    // deselect all items.
    useEffect(() => {
        if (actions.bulkSelectedIds.size === 0)
            return;
        const handler = (e) => {
            const target = e.target;
            if (target.closest(".prd-tree") ||
    ...
    // Branded header
    h("div", { class: "view-header" }, h(BrandedHeader, { product: "rex", title: "Rex", class: "branded-header-rex" }), h("h2", { class: "section-header" }, "Tasks")), 
    // Deep-link error banner
    deepLinkError
        ? h("div", { class: "prd-deep-link-error", role: "alert" }, h("span", null, deepLinkError), h("button", {
    ...
    h("div", { class: "rex-command-bar" }, h("button", {
    ...
    h("div", { class: "prd-filter-bar" }, h("div", { class: "prd-search-row" }, h("input", {
        ref: searchInputRef,
        ...
        ? h("span", { class: "prd-search-count", "aria-live": "polite" }, `${searchResult.matchCount} match${searchResult.matchCount !== 1 ? "es" : ""}`)
        : null, searchQuery || hasFacets
        ? h("button", {
