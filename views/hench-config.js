    ...
    try {
        ...
        switch (field.path) {
            ...
            case "guard.blockedPaths":
                return `${value.length} blocked path patterns`;
            case "guard.allowedCommands":
                return `Allowed: ${value.join(", ")}`;
            default:
                return "";
        }
    }
...
function TagListEditor({ value, onChange, disabled }) {
    ...
    return h("div", { class: "hench-config-tags" }, h("div", { class: "hench-config-tag-list" }, ...tags.map((tag, i) => h("span", { key: `${tag}-${i}`, class: "hench-config-tag" }, h("span", { class: "hench-config-tag-text" }, tag), !disabled
        ? h("button", {
            type: "button",
        ...
        : null))), !disabled
        ? h("div", { class: "hench-config-tag-input-row" }, h("input", {
            type: "text",
...
function FieldControl({ field, value, onChange, error }) {
    ...
    switch (field.type) {
        ...
        case "boolean":
            return h("label", { class: "hench-config-toggle" }, h("input", {
                type: "checkbox",
                checked: value === "true",
                onChange: (e) => onChange(String(e.target.checked)),
            }), h("span", { class: "hench-config-toggle-slider" }), h("span", { class: "hench-config-toggle-label" }, value === "true" ? "Enabled" : "Disabled"));
        case "array":
...
function FieldEditor({ field, editValue, error, onFieldChange }) {
    ...
    return h("div", { class: `hench-config-field${!field.isDefault ? " modified" : ""}${dirty ? " dirty" : ""}` }, h("div", { class: "hench-config-field-header" }, h("div", { class: "hench-config-field-label" }, h("span", { class: "hench-config-field-name" }, field.label), !field.isDefault ? h("span", { class: "hench-config-modified-badge" }, "modified") : null, dirty ? h("span", { class: "hench-config-dirty-badge" }, "unsaved") : null), h("div", { class: "hench-config-field-actions" }, h("span", { class: "hench-config-field-path" }, field.path), dirty
        ? h("button", {
            type: "button",
        ...
        : null)), h("p", { class: "hench-config-field-desc" }, field.description), 
    // Form control
    h("div", { class: "hench-config-control-row" }, h(FieldControl, { field, value: editValue, onChange: handleChange, error })), 
    // Impact preview or current impact
    (dirty && previewImpact)
        ? h("div", { class: "hench-config-preview" }, h("span", { class: "hench-config-preview-label" }, "Impact: "), h("span", null, previewImpact))
        : h("div", { class: "hench-config-impact" }, h("span", null, field.impact)), 
    // Validation error
    error
...
function CategorySection({ category, fields, editValues, errors, onFieldChange }) {
    ...
    return h("div", { class: "hench-config-category" }, h("div", { class: "hench-config-category-header" }, h("span", { class: "hench-config-category-icon" }, meta.icon), h("div", null, h("h3", { class: "hench-config-category-title" }, meta.label), h("p", { class: "hench-config-category-desc" }, meta.description))), h("div", { class: "hench-config-fields" }, ...fields.map((field) => h(FieldEditor, {
        key: field.path,
        field,
...
function ChangesSummary({ pendingChanges, onSave, onDiscard, saving }) {
    if (pendingChanges.length === 0)
        return null;
    return h("div", { class: "hench-config-changes-panel" }, h("div", { class: "hench-config-changes-header" }, h("span", { class: "hench-config-changes-title" }, `${pendingChanges.length} unsaved change${pendingChanges.length > 1 ? "s" : ""}`)), h("div", { class: "hench-config-changes-list" }, ...pendingChanges.map((change) => h("div", { key: change.path, class: "hench-config-change-item" }, h("div", { class: "hench-config-change-label" }, change.label), h("div", { class: "hench-config-change-diff" }, h("span", { class: "hench-config-change-old" }, change.oldDisplay || "(empty)"), h("span", { class: "hench-config-change-arrow" }, "\u2192"), h("span", { class: "hench-config-change-new" }, change.newDisplay || "(empty)")), h("div", { class: "hench-config-change-impact" }, change.impact)))), h("div", { class: "hench-config-changes-actions" }, h("button", {
        type: "button",
        class: "hench-config-discard-btn",
        onClick: onDiscard,
        disabled: saving,
    }, "Discard All"), h("button", {
        type: "button",
...
// ── Toast notification ───────────────────────────────────────────────
function SaveToast({ changes }) {
    if (changes.length === 0)
        return null;
    return h("div", { class: "hench-config-toast" }, h("span", { class: "hench-config-toast-icon" }, "\u2714"), h("span", null, `Saved ${changes.length} change${changes.length > 1 ? "s" : ""}`));
}
// ── Main view ────────────────────────────────────────────────────────
export function HenchConfigView() {
    ...
    const handleFieldChange = useCallback((path, rawValue) => {
    ...
    const handleSaveAll = useCallback(async () => {
        if (!data || pendingChanges.length === 0 || hasValidationErrors)
            return;
        setSaving(true);
        setError(null);
        try {
    ...
    if (loading) {
        return h("div", { class: "hench-config-container" }, h("div", { class: "loading" }, "Loading configuration..."));
    }
    if (error && !data) {
        return h("div", { class: "hench-config-container" }, h(BrandedHeader, { product: "hench", title: "Workflow Configuration" }), h("div", { class: "hench-config-error-state" }, h("p", null, error), h("p", { class: "hench-config-error-hint" }, "Make sure ", h("code", null, ".hench/"), " exists. Run ", h("code", null, "hench init"), " to create it.")));
    }
    if (!data)
        return null;
    ...
    return h("div", { class: "hench-config-container" }, h("div", { class: "hench-config-header" }, h(BrandedHeader, { product: "hench", title: "Workflow Configuration" }), modifiedCount > 0
        ? h("span", { class: "hench-config-modified-count" }, `${modifiedCount} field${modifiedCount > 1 ? "s differ" : " differs"} from defaults`)
        : null), 
    // Save error banner
    error
...
//# sourceMappingURL=hench-config.js.map