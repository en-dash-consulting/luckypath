...
function validateFieldValue(value, schema) {
...
function groupFields(schema) {
    ...
    const groupKeys = Array.from(grouped.keys()).sort((a, b) => {
        if (a === null)
            return 1; // ungrouped fields go last
        if (b === null)
            return -1;
...
function DynamicField({ fieldKey, schema, value, error, configured, masked, onInput, }) {
    ...
    const handleSelect = useCallback((e) => {
        onInput(fieldKey, e.target.value);
    }, [fieldKey, onInput]);
    return h("div", {
        class: `intg-field${hasError ? " intg-field-error" : ""}`,
    }, 
    // Label
    h("label", { class: "intg-field-label" }, schema.label ?? fieldKey, configured && (schema.sensitive ? masked : undefined)
        ? h("span", { class: "intg-field-badge" }, "configured")
        : null, schema.required
        ? h("span", { class: "intg-field-required" }, "*")
        : null), 
    // Help text
    schema.helpText
        ? h("p", { class: "intg-field-help" }, schema.helpText, schema.docUrl
            ? h(Fragment, null, " ", h("a", {
                href: schema.docUrl,
                target: "_blank",
    ...
    // Input element
    inputType === "checkbox"
        ? h("label", { class: "intg-checkbox-wrapper" }, h("input", {
            type: "checkbox",
        ...
        : inputType === "select"
            ...
            }, h("option", { value: "" }, "— Select —"), (schema.options ?? []).map((opt) => h("option", { key: opt.value, value: opt.value, title: opt.description }, opt.label)))
            : inputType === "textarea"
                ? h("textarea", {
                ...
                : inputType === "password"
                    ? h("div", { class: "intg-input-row" }, h("input", {
...
function IntegrationDetail({ schema, onBack, }) {
    ...
    const handleFieldInput = useCallback((key, value) => {
    ...
    // ── Render ──────────────────────────────────────────────────────────
    if (loading) {
        return h("div", { class: "intg-container" }, h("div", { class: "loading" }, `Loading ${schema.name} configuration...`));
    }
    ...
    // ── Header ──────────────────────────────────────────────────────
    h("div", { class: "intg-header" }, h(BrandedHeader, { product: "rex", title: `${schema.name} Integration` }), h("p", { class: "intg-subtitle" }, schema.description)), 
    // ── Error banner ─────────────────────────────────────────────────
    error
    ...
    // ── Save success toast ───────────────────────────────────────────
    saveSuccess
        ? h("div", { class: "intg-toast" }, h("span", { class: "intg-toast-icon" }, "\u2714"), "Configuration saved")
        : null, 
    // ── Form fields grouped ──────────────────────────────────────────
    groupedFields.map(({ groupKey, group, fields }) => h("div", { key: groupKey ?? "__ungrouped", class: "intg-section" }, group
        ? h("h3", { class: "intg-section-title" }, group.icon ? h("span", { class: "intg-section-icon" }, group.icon) : null, group.label)
        : null, group?.description
        ? h("p", { class: "intg-section-desc" }, group.description)
        : null, fields.map(({ key, schema: fieldSchema }) => h(DynamicField, {
        key,
        fieldKey: key,
        schema: fieldSchema,
    ...
    // ── Actions ──────────────────────────────────────────────────────
    h("div", { class: "intg-actions" }, h("div", { class: "intg-actions-primary" }, h("button", {
        type: "button",
        class: "intg-save-btn",
        onClick: handleSave,
        disabled: saving || !hasValues,
    }, saving ? "Saving..." : "Save Configuration")), isConfigured
        ? h("div", { class: "intg-actions-danger" }, confirmRemove
            ? h(Fragment, null, h("span", { class: "intg-confirm-text" }, `Remove ${schema.name} config?`), h("button", {
                type: "button",
    ...
    isConfigured && Object.keys(config?.envVars ?? {}).length > 0
        ? h("div", { class: "intg-section" }, h("div", { class: "intg-env-hint" }, h("span", { class: "intg-env-hint-icon" }, "\u{1F512}"), h("div", { class: "intg-env-hint-content" }, h("p", { class: "intg-env-hint-title" }, "Credentials stored securely"), h("p", { class: "intg-env-hint-desc" }, "Sensitive fields are redacted on disk. Set these environment variables at runtime:"), Object.entries(config.envVars).map(([key, envVar]) => h("code", { key, class: "intg-env-hint-code" }, `export ${envVar}="your-${key}"`)))))
        : null, 
    // ── Setup guide ──────────────────────────────────────────────────
    schema.setupGuide && schema.setupGuide.length > 0
        ? h("div", { class: "intg-section intg-help" }, h("h3", { class: "intg-section-title" }, h("span", { class: "intg-section-icon" }, "\u2139"), "Setup Guide"), h("ol", { class: "intg-steps" }, schema.setupGuide.map((step, i) => h("li", { key: i }, step))), schema.docsUrl
            ? h("p", { class: "intg-docs-link" }, h("a", {
                href: schema.docsUrl,
                target: "_blank",
                rel: "noopener noreferrer",
...
function IntegrationCard({ schema, configured, onClick, }) {
    return h("div", {
    ...
    }, h("div", { class: "intg-card-icon" }, schema.icon ?? "\u{1F50C}"), h("div", { class: "intg-card-content" }, h("div", { class: "intg-card-header" }, h("h3", { class: "intg-card-name" }, schema.name), configured
        ? h("span", { class: "intg-card-badge" }, "\u2714 Configured")
        : null, schema.builtIn
        ? h("span", { class: "intg-card-builtin" }, "Built-in")
        : null), h("p", { class: "intg-card-desc" }, schema.description), h("div", { class: "intg-card-meta" }, h("span", null, `${Object.keys(schema.fields).length} fields`), schema.supportsConnectionTest
        ? h("span", null, "\u2022 Connection test")
        : null)), h("span", { class: "intg-card-arrow" }, "\u203A"));
}
// ── Main view ────────────────────────────────────────────────────────
export function IntegrationConfigView() {
    ...
    if (loading) {
        return h("div", { class: "intg-container" }, h("div", { class: "loading" }, "Loading integrations..."));
    }
    return h("div", { class: "intg-container" }, h("div", { class: "intg-header" }, h(BrandedHeader, { product: "rex", title: "Integrations" }), h("p", { class: "intg-subtitle" }, "Connect external services to sync PRD data bidirectionally.")), error
        ? h("div", { class: "intg-error-banner" }, error)
        : null, h("div", { class: "intg-list" }, schemas.map((s) => h(IntegrationCard, {
        key: s.id,
        schema: s,
        configured: configuredIds.has(s.id),
        onClick: () => setSelectedId(s.id),
    }))), schemas.length === 0
        ? h("div", { class: "intg-empty" }, h("p", null, "No integrations available."), h("p", { class: "intg-empty-hint" }, "Integration schemas are registered by the rex adapter system."))
        : null);
}
