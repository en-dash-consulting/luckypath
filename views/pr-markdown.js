...
function parseStoredViewMode(value) {
...
function parseMarkdownPayload(markdown) {
    if (typeof markdown !== "string")
        return null;
    return markdown.trim().length > 0 ? markdown : null;
}
function formatCachedTimestamp(ts) {
    if (!ts)
        return null;
...
function isPermissionDeniedClipboardError(error) {
...
function renderMarkdownPreview(markdown) {
    ...
    while (i < lines.length) {
        ...
        if (/^```/.test(trimmed)) {
            ...
            if (i < lines.length)
                i += 1;
            blocks.push(h("pre", { class: "pr-markdown-preview-code", key: `c-${key += 1}` }, h("code", { class: language ? `language-${language}` : undefined }, codeLines.join("\n"))));
            continue;
        }
...
export function PRMarkdownView() {
    ...
    const [cacheStatus, setCacheStatus] = useState("missing");
    const [generatedAt, setGeneratedAt] = useState(null);
    const [staleAfterMs, setStaleAfterMs] = useState(null);
    const [latestSignature, setLatestSignature] = useState(null);
    const [copyState, setCopyState] = useState("idle");
    const [copyErrorKind, setCopyErrorKind] = useState(null);
    const [viewMode, setViewMode] = useState(() => readStoredViewMode());
    const inFlightRef = useRef(false);
    const hasLoadedRef = useRef(false);
    const loadedMarkdownSignatureRef = useRef(null);
    const hasSuccessfulMarkdownRef = useRef(false);
    const copyFeedbackTimerRef = useRef(null);
    const loadMarkdown = useCallback(async () => {
        const res = await fetch("/api/sv/pr-markdown");
        if (!res.ok)
            throw new Error(`Failed to load PR markdown (${res.status})`);
        const json = await res.json();
        return parseMarkdownPayload(json.markdown);
    }, []);
    const applyStatePayload = useCallback((json) => {
    ...
    const refreshFromState = useCallback(async (forceMarkdownFetch = false) => {
        ...
        try {
            ...
            if (!res.ok)
                throw new Error(`Failed to load PR markdown state (${res.status})`);
            const json = await res.json();
    ...
    const setCopyFeedback = useCallback((nextState) => {
    ...
    const fallbackTitle = availability === "unsupported"
        ...
        : availability === "no-repo"
            ...
            : availability === "error"
                ...
                : "";
    return h("div", { class: "pr-markdown-container" }, h("div", { class: "view-header" }, h(BrandedHeader, { product: "sourcevision", title: "SourceVision", class: "branded-header-sv" }), h("h2", { class: "section-header" }, "PR Markdown")), h("p", { class: "section-sub" }, "PR markdown is generated automatically during ", h("code", null, "sourcevision analyze"), ". Run an analysis to update."), loading
        ? h("div", { class: "loading", role: "status", "aria-live": "polite" }, "Loading PR markdown...")
        : null, !loading && error
        ? h("div", { class: "card pr-markdown-empty" }, h("h3", { class: "section-header-sm" }, "Unable to load PR markdown"), h("p", null, error))
        : null, !loading && !error && availability !== "ready"
        ? h("div", { class: "card pr-markdown-empty", role: "status", "aria-live": "polite" }, h("h3", { class: "section-header-sm" }, fallbackTitle), h("p", null, message ?? "PR markdown is unavailable in this environment."))
        : null, !loading && !error && availability === "ready" && warning
        ? h("div", { class: "card pr-markdown-warning", role: "status", "aria-live": "polite" }, h("h3", { class: "section-header-sm" }, "Partial git metadata only"), h("p", null, warning), h("p", null, message ?? "PR markdown generation is degraded."), h("p", { class: "section-sub pr-markdown-warning-meta" }, `Base range: ${baseRange ?? "unresolved"} | Signature: ${latestSignature ? latestSignature.slice(0, 12) : "unknown"}`))
        : null, !loading && !error && availability === "ready" && !markdown
        ? h("div", { class: "card pr-markdown-empty", role: "status", "aria-live": "polite" }, h("h3", { class: "section-header-sm" }, "No PR markdown available"), h("p", null, "PR markdown has not been generated yet."), h("p", null, "Run ", h("code", null, "sourcevision analyze"), " to generate PR markdown automatically."))
        : null, !loading && !error && availability === "ready" && cacheStatus === "stale" && markdown
        ? h("div", { class: "card pr-markdown-warning", role: "status", "aria-live": "polite" }, h("h3", { class: "section-header-sm" }, "Cached PR markdown is stale"), h("p", null, `This cached artifact is older than ${formatStaleDuration(staleAfterMs)}.`), h("p", null, "Run ", h("code", null, "sourcevision analyze"), " to regenerate."), h("p", { class: "section-sub pr-markdown-warning-meta" }, `Generated at: ${formatCachedTimestamp(generatedAt) ?? "unknown"}`))
        : null, !loading && !error && markdown
        ? h("div", { class: "card pr-markdown-success", role: "status", "aria-live": "polite" }, h("h3", { class: "section-header-sm" }, "PR markdown ready"), generatedAt
            ? h("p", { class: "section-sub pr-markdown-gen-meta" }, cacheStatus === "fresh" ? "\u2713 " : "", `Generated ${formatCachedTimestamp(generatedAt) ?? "at unknown time"}`, cacheStatus === "fresh" ? " — fresh" : "")
            : null, h("div", { class: "pr-markdown-mode-toggle", role: "group", "aria-label": "PR markdown view mode" }, h("button", {
            type: "button",
        ...
        }, "Raw")), viewMode === "preview"
            ? h("section", { class: "pr-markdown-panel", id: "pr-markdown-panel-preview" }, h("h4", { class: "section-header-sm" }, "Preview"), h("div", { class: "pr-markdown-preview" }, renderMarkdownPreview(markdown)))
            : h("section", { class: "pr-markdown-panel", id: "pr-markdown-panel-raw" }, h("div", { class: "pr-markdown-raw-header" }, h("h4", { class: "section-header-sm" }, "Raw Markdown"), h("button", {
                type: "button",
                class: "btn pr-markdown-copy-btn",
                onClick: () => { void handleCopyRawMarkdown(); },
            }, copyState === "success" ? "Copied" : "Copy Markdown")), h("pre", { class: "pr-markdown-raw" }, markdown), h("p", { class: "section-sub pr-markdown-copy-feedback", role: "status", "aria-live": "polite" }, copyFeedbackMessage)))
        : null);
}
//# sourceMappingURL=pr-markdown.js.map