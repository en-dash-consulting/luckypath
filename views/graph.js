...
function getInitialGraphVisible() {
    try {
...
export function Graph({ data, onSelect, selectedFile, selectedZone, navigateTo }) {
    ...
    const handleNodeDblClick = useCallback((path) => {
        if (navigateTo) {
    ...
    const handleZoneLegendClick = useCallback((zoneId) => {
        if (!graphRef.current)
            return;
    ...
    const handleToggleGraph = useCallback(() => {
        setGraphVisible((prev) => {
            const next = !prev;
            try {
                localStorage.setItem(GRAPH_VISIBLE_KEY, String(next));
            }
            catch { /* noop */ }
            if (!next) {
    ...
    useEffect(() => {
        if (!svgRef.current || !imports || initialized)
            return;
        ...
        const links = imports.edges.map((e) => ({
            source: e.from,
            target: e.to,
    ...
    useEffect(() => {
        ...
        const match = graphRef.current.nodes.find((n) => n.id.toLowerCase().includes(q));
        if (match) {
    ...
    return h("div", null, h("div", { class: "view-header" }, h(BrandedHeader, { product: "sourcevision", title: "SourceVision", class: "branded-header-sv" }), h("h2", { class: "section-header" }, "Import Graph")), h("p", { class: "section-sub" }, `${imports.edges.length} edges between ${nodeCount} files`, zones ? ` across ${zones.zones.length} zones` : ""), 
    // Graph visibility toggle
    h("div", { class: "graph-visibility-bar" }, h("button", {
    ...
    }, graphVisible ? "\u2713 Graph Visible" : "Show Graph"), !graphVisible
        ? h("span", { class: "graph-visibility-hint" }, "The force-directed graph is hidden to improve performance. Click to enable.")
        : null), 
    // Graph content: shown only when toggle is on
    graphVisible
        ? h("div", null, 
        // Search bar
        h("div", { class: "graph-search-bar" }, h("input", {
            type: "text",
            class: "filter-input",
            placeholder: "Search nodes...",
            value: graphSearch,
            onInput: (e) => setGraphSearch(e.target.value),
        })), h("div", { class: "graph-container", style: "position: relative;" }, h("svg", { ref: svgRef }), 
        // Label toggle button
        h("button", {
        ...
        // Zoom controls
        h("div", { class: "graph-zoom-controls" }, h("button", {
        ...
        legendItems.length > 0
            ? h("div", { class: "graph-legend" }, h("div", { class: "graph-legend-title", style: "font-size: 10px; color: var(--text-dim); margin-bottom: 4px; opacity: 0.7;" }, "Click to toggle zone"), legendItems.map((item) => h("div", {
                key: item.name,
                ...
                onClick: () => {
                    ...
                    if (zone)
                        handleZoneLegendClick(zone.id);
                },
            }, h("span", { class: "graph-legend-dot", style: `background: ${item.color};` }), item.name)))
            : null))
        : // Collapsed placeholder with summary stats
            h("div", { class: "graph-hidden-placeholder" }, h("div", { class: "graph-hidden-icon" }, "\u2B95"), h("div", { class: "graph-hidden-stats" }, h("span", { class: "stat-card" }, h("span", { class: "value" }, String(nodeCount)), h("span", { class: "label" }, "Files")), h("span", { class: "stat-card" }, h("span", { class: "value" }, String(imports.edges.length)), h("span", { class: "label" }, "Edges")), zones
                ? h("span", { class: "stat-card" }, h("span", { class: "value" }, String(zones.zones.length)), h("span", { class: "label" }, "Zones"))
                : null)));
}
