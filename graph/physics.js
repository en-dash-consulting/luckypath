 ...
 */
// ── Force parameters ────────────────────────────────────────────────────────
export function computeForceParams(nodeCount) {
...
export function hashPosition(id) {
    let h = 0;
...
export function initZoneClusteredPositions(nodes, width, height) {
 ...
 */
export function applyZoneCentroidRepulsion(nodes, centroids, strength, alpha) {
...
export function buildQuadTree(nodes, nodeCount) {
    if (nodeCount === 0)
        return null;
    ...
    x1 += pad;
    y1 += pad;
    const root = { cx: 0, cy: 0, mass: 0, x0, y0, x1, y1, children: null, nodeIdx: -1 };
    function insert(qt, idx, nx, ny) {
    ...
    function insertIntoChild(qt, idx, nx, ny, midX, midY) {
...
export function bhRepulsion(qt, idx, nx, ny, a, nodes, repulsionStrength, bhTheta) {
    if (!qt || qt.mass === 0)
        return;
    ...
    for (const child of qt.children) {
        if (child)
            bhRepulsion(child, idx, nx, ny, a, nodes, repulsionStrength, bhTheta);
    }
}
export function tick(sim, callbacks) {
    const { nodes, resolvedLinks, width, height } = sim;
    const nodeCount = nodes.length;
    const { alphaDecay, velocityDecay, repulsionStrength, centerGravityStrength, linkRestLength, useBH, bhTheta, crossZoneLinkMultiplier, zoneCohesionStrength, zoneRepulsionStrength, } = computeForceParams(nodeCount);
