...
import { registerPollingSource, } from "./polling-state.js";
// ─── Constants ───────────────────────────────────────────────────────────────
/**
...
function deactivatePoller(entry) {
    if (entry.timerId === null)
 ...
 */
export function startPollingManager() {
    if (started)
        stopPollingManager();
    started = true;
    suspended = false;
 ...
 */
export function stopPollingManager() {
    cleanupOwnResources();
    started = false;
 ...
 */
export function registerPoller(key, callback, intervalMs) {
 ...
 */
export function isPollerActive(key) {
    const entry = pollers.get(key);
 ...
 */
export function getRegisteredPollers() {
    const result = [];
    for (const entry of pollers.values()) {
        result.push({
            key: entry.key,
 ...
 */
export function getPollerCount() {
    return pollers.size;
}
/**
 ...
 */
export function resetPollingManager() {
    // Clear all pollers.
    for (const entry of pollers.values()) {
        deactivatePoller(entry);
    }
