 ...
 */
export function useFileEdges(edges, boxes, expandedZones, zoneById, fileConnections, fileToFileMap) {
    const fileEdgeElements = useMemo(() => {
        ...
        for (const edge of edges) {
            ...
            if (fromExpanded && fromZone?.subZones?.length)
                continue;
            if (toExpanded && toZone?.subZones?.length)
                continue;
...
// ── Internal helpers ──────────────────────────────────────────────────
function buildBothExpandedEdges(elements, fromZone, toZone, fromBox, toBox, edge, fileToFileMap, fileConnections) {
...
function buildOneExpandedEdges(elements, expandedZone, expandedBox, otherBox, otherZoneId, edgeColor, fileConnections) {
...
//# sourceMappingURL=use-file-edges.js.map