...
function normalizeWeeklyBudgetResolution(value) {
 ...
 */
function applyUtilizationToTaskUsage(serverUsage, weeklyBudget) {
    const byTask = {};
...
function applyWeeklyBudget(taskUsageById, weeklyBudget) {
    const next = {};
 ...
 */
export function usePRDData(prdData) {
    const [data, setData] = useState(prdData ?? null);
    const [loading, setLoading] = useState(!prdData);
    const [error, setError] = useState(null);
    const [taskUsageById, setTaskUsageById] = useState({});
    ...
    const prdPipeline = useRef(createFetchPipeline(async () => {
        try {
    ...
    const usagePipeline = useRef(createFetchPipeline(async () => {
        ...
        if (utilizationResult.status === "fulfilled" && utilizationResult.value.ok) {
            try {
                const json = await utilizationResult.value.json();
        ...
        if (taskUsageResult.status === "fulfilled" && taskUsageResult.value.ok) {
            try {
                const json = await taskUsageResult.value.json();
...
//# sourceMappingURL=use-prd-data.js.map