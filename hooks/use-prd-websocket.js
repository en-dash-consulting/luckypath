 ...
 */
export function usePRDWebSocket({ setData, fetchPRDData, fetchTaskUsage }) {
    useEffect(() => {
        let ws = null;
        ...
        const pipeline = createWSPipeline({
            ...
            onMessage: (msg) => {
                if (msg.type === "rex:item-updated" && msg.itemId && msg.updates) {
                    updateGate.schedule(setData, (prev) => {
                        if (!prev)
                            return prev;
            ...
            onFlush: (batch) => {
                updateGate.flush();
                const needsReconciliation = batch.types.has("rex:item-updated") ||
        ...
        try {
            ...
            ws.onmessage = (event) => {
                try {
                    ...
                    if (!bufferGate.accept())
                        return; // Tab hidden — drop message
                    pipeline.push(msg);
                }
                catch {
...
//# sourceMappingURL=use-prd-websocket.js.map