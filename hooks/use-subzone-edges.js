 ...
 */
function subZoneRowY(box, subZones, targetIndex, expandedSubZoneIds) {
...
export function useSubZoneEdges(edges, boxes, expandedZones, expandedSubZones, zoneById, fileConnections) {
    ...
    const hiddenZoneEdges = useMemo(() => {
        ...
        for (const edge of edges) {
            ...
            if (fromHasSubZones || toHasSubZones) {
                const hasSubEdges = subZoneEdgeElements.some((se) => se.key.includes(edge.from) || se.key.includes(edge.to));
                if (hasSubEdges)
...
//# sourceMappingURL=use-subzone-edges.js.map