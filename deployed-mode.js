...
export function getDeployedConfig() {
...
export function installFetchAdapter() {
    ...
    globalThis.fetch = function deployedFetch(input, init) {
        ...
        if (method !== "GET" && (pathname.startsWith("/api/") || pathname.startsWith("/data"))) {
            return Promise.resolve(new Response(JSON.stringify({ error: "Not available in deployed mode" }), {
                status: 405,
        ...
        if (pathname === "/data/status") {
            return Promise.resolve(new Response(JSON.stringify({ mtimes: {} }), {
                status: 200,
                headers: { "Content-Type": "application/json" },
            }));
        }
...
//# sourceMappingURL=deployed-mode.js.map